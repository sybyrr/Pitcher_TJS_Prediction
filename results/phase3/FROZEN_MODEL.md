...
> **명세 정정 (2026-07-13, KBO 이전 감사):** 위 `vel_trend`의 "직전
> 시즌" 표현은 구현과 다르다. 동결 scorer의 실제 기준은 현재 연도보다
> 앞선 **모든 시즌의 투구수 가중 평균 구속**이며, 없으면 t−30일 이전
> 전체 이력 평균으로 fallback한다. `start_share`는 공식 GS가 아니라 최근
> 365일 등판 중 50구 이상 경기 비율이다. KBO feature crosswalk는 이 구현
> 정의를 사용한다. 이는 계수·상태 변경이 아니라 문서 명세 오류 정정이다.
