...
2. **분류 성능의 상당 부분은 fill-artifact 지름길이다.** 이력 길이 스칼라
   ...
   AUC 0.816의 실질 신호가 남는다** (v9 — 저자 데이터+동결 라벨 기준).
   **[정정 2026-07-08: 자체 재추출+성숙 라벨 기준 정직 회고 수치는
   ROC ~0.60 — 블록 6 참조. 0.816은 재현 체크포인트로만 유효.]**
...
### 블록 5 — 2.6 설정 재감사 + 역할(선발/불펜) 도입 (2026-07-08, 사용자 지적)

**계기**: 5일 bin 시퀀스를 요약 스칼라로 바꾸면서(S2) 역할 정보를 담을
그릇이 사라짐 — 같은 feature 값이 역할별로 정반대 의미(선발 dsl=5 정상,
불펜 dsl=5 이상). 2.6 전환 설정 12개(S1~S12) 전면 재감사 결과: 9개 유지
정당, 재검토 3개 = **S4 역할 pooling(이 블록에서 해결), S9 단일 시대
test(R1 담당), S3 콘텐츠-only 미검정(B' 담당)**.

**역할 진단** (`role_diag.py`): 역할 프록시(직전 365일 경기 중 50구 이상
비중, 0.5 경계)는 깨끗 — 스윙맨 애매 구간 3.6%뿐. 교란 실재: pc_chronic
중앙값 선발 4.73 vs 불펜 0.99(**5배**). **Simpson 구조**: 선발 base rate가
불펜의 ~2배(test H=90 2.22% vs 1.13%) — 역할 간 "사용량↑=위험↑" +
역할 내 "사용량 감소=위험"이 pooled 계수에서 상쇄되고 있었음.

**역할 모델** (`role_models.py`, 동결 프로토콜·짝지은 부트스트랩):
- **+start_share additive가 승자**: dPR **+0.0133/+0.0169** (양 H에서
  CI 0 상방 배제 — 이 프로젝트에서 처음으로 "유의한 개선"), ROC는
  +0.028p이나 CI 0 포함(검정력), event recall H=150 대폭 개선
  (@10 2→8, @50 13→15), H=90 @50은 하락(19→13). 상호작용·역할 내
  표준화는 additive 대비 이득 없음 — start_share 하나로 충분.
- 계수로 Simpson 확증: start_share +0.29, 역할 통제 후 pc_chronic
  −0.185→−0.38(2배) — 상쇄가 풀림.
- 성격 규정(정직): 이 개선은 "선발이 TJS를 2배 받는다"는 **역할 간
  prevalence 효과**이지 새 급성 신호가 아님. 역할 내 스토리("사용량
  감소=위험")는 오히려 강화됨.

**결정**: canonical baseline을 **M-role = M0'' + start_share (6 feature)**
로 **잠정 승격** — 단 같은 test fold의 반복 조회 누적을 감안해 R1
(rolling-origin)에서 재확정을 조건으로 함. **[확정 2026-07-08: R1
rolling-origin 기준 (a)(b) PASS — 전 fold ROC>0.55, dPR/dROC 6/6 fold
양(+), 2023 이득 최대. plan_progress.md R 블록 로그 참조.]** 계획 v3의 R2는 역할 부분이
해결됐으므로 **나이(age)만 잔여**. KBO 함의: 역할 공변량은 리그 간 역할
구조 차이(외국인 선발, 불펜 운용)를 흡수하는 장치로 이전성도 개선.
산출물: `results/phase26/ROLE_MODELS.md`, `role_models_results.csv`,
scripts 2종.

### 블록 6 — R 블록 (계획 v3, 2026-07-08); 회고 대표 수치 하향 정정

R1/R2/R4 요약은 `plan_progress.md` R 블록 로그가 canonical. 여기엔 **회고
대표 수치의 정정**만 기록(제안서 영향).

**R3 — v9 회고 수치 재측정 (라이브 라벨 + 자체 재추출).** seed-paired
(seeds 100..1000):
| 변형 | 데이터 / 라벨 | ROC | PR | test 부상 |
|---|---|---|---|---|
| v9 (기존 헤드라인) | 저자 data / 스냅샷(동결) | **0.816** | 0.608 | 43 |
| v9o_snap | 우리 재추출 / 스냅샷 | 0.675 | 0.386 | 48 |
| v9o_live | 우리 재추출 / 라이브(성숙) | **0.601** | 0.405 | 61 |

분해: 저자→우리 재추출 −0.14 (추출 충실도 갭 — 지름길 제거한 저SNR
영역에서 per-pitcher 오프셋 노이즈가 신호 대비 커짐; 전체창 변형에선
ours≈authors였음), 스냅샷→라이브 −0.074 (**paired Wilcoxon p=0.002, 전
10 seed 하락**). PR은 소폭 상승(prevalence↑ 상쇄)이나 비유의.

**정정 (정직 보고)**: 세션 초 가설("라벨 교정하면 0.816이 오를 것")은
**틀렸다.** 교정은 수치를 **낮춘다** — 새로 드러난 양성이 대부분 2024
수술자(먼 미래·약신호 케이스)라 더 어렵기 때문. **자체 재현 가능·성숙
라벨 기준 정직한 회고 수치 = ROC ~0.60** (0.816은 저자 데이터+동결 라벨
재현 체크포인트로만 유효).

**서사 재편**: 이로써 "회고 0.82 강 → 전향 0.55 붕괴"라는 기존 프레임이
사라진다. 정직 회고(~0.60)와 정직 전향(M-role ~0.64)이 **같은 대역**으로
수렴 — 신호는 modest하지만 회고/전향에 걸쳐 **안정적**. 단 두 수치는
코호트·feature·모델·base rate가 달라(회고 pitch-shape/ViT/~31% vs 전향
workload/LR/1.5%) 대역 수준 비교이지 짝 비교 아님. 제안서 함의: "강한
회고 수치" 대신 "공개 feature에서 modest·프레임 안정 신호"로 리드,
Tier-2는 "이 신호를 유의미하게 올리는가"의 검정(B)으로. 산출물:
`results/phase2/v9o_{snap,live}.csv`, R3 코드 `data/final_df_live.csv` 등.

### 블록 7 — B 블록: feature-tier × 모델 ablation (2026-07-10)

질문 ① Tier-2 트래킹(spin·extension·release point·구종 구성)이 M-role을
올리는가 ② tree(HistGBM)가 LR보다 나은가 — **둘 다 아니오**. 상세·표는
`results/phase3/B_TIER_MODELS.md` canonical, 여기엔 판정만.

설계: {F1=M-role 6, F2=F1+Tier-2 12, F3=콘텐츠-only} × {LR 동결 프로토콜,
HGB(valid 전용 그리드)}, H∈{90,150} B=0, paired bootstrap 1000 공유,
M-role 재현 게이트 1e-7 통과.

1. **Tier-2 증분 널 (3중 확인).** 달력창 널 → NaN 35-49%(4·5월 결정
   시점이 오프시즌 조회) 희석 우려 → 경기창(최근 5/15경기, NaN<8%) 재구축
   + 관측 부분집합 대조까지 전부 ΔROC CI가 0 포함 또는 음수. 존재해도
   **+0.05 초과 증분은 배제** (LR 증분 CI 상한 +0.02~+0.06).
2. **tree 널.** 동일 6 feature에서 HGB≈LR(±0.005 n.s.), feature 확장 시
   유의 악화(ΔROC −0.07~−0.08 EXCL0). 확장 그리드 48설정(depth 1, lr .01,
   l2 10까지)도 F1 ±0.011 비일관 / F2g −0.06~−0.09, H150 valid 0.70→test
   0.63 전이 실패(selection noise). **추가 hyperparameter 탐색 근거 없음.**
3. **콘텐츠-only ROC 0.49-0.55** — 신호 대부분은 사용량·역할·간격.
   콘텐츠만으로는 사용량 정보를 대체 불가(vs M-role ΔROC −0.10~−0.15).
4. G3 함의: 트래킹 논거는 "MLB에서 증분 입증"이 아니라 "MLB 규모에선 검출
   한계 이하 → KBO 내부 라벨과 결합해야 검정 가능(데이터 접근 = 검정력)"로
   정식화. **M-role이 최종 모델 후보로 유지.** 양성 결과 없음 → 사전
   규칙대로 rolling-origin 생략.

### 블록 8 — B' (ii)(iii): 사용량 정상 하위집단 + lead 분해 (2026-07-10)

질문: M-role 포착 = "구단이 이미 아는 사용량 감소"의 재탕인가. 상세
canonical: `results/phase3/B_PRIME_LEAD.md`. 판정 3줄:

1. **급성 감소 flag 단독은 window 수준에서 무판별** (감소側 base rate 비
   0.97~1.22×) — M-role의 판별력은 만성 사용량 × 역할 프로파일이 지고
   있음 (블록 5 Simpson·E3 급성 널과 정합).
2. **H150 포착의 60%(9/15)는 포착 시점 사용량 정상, lead 중앙값 72일**;
   사용량 정상 하위집단 내에서도 ROC 0.663 [0.582, 0.751] 유지. H90은
   셧다운 중첩이 커서(4/13 정상, ROC 0.59 CI가 0.5 포함) 임박 확인용.
3. 해석: 급성 셧다운 = 90일 임박 신호 / 만성 프로파일 = 150일·lead ~72일
   조기 신호 → "구단이 이미 안다" 반론에 대한 실측 응답 확보. 단 포착
   n=13~15로 CI 넓음 — 분수는 점추정.

### 블록 9 — P 블록: 성능 개선 실행 (2026-07-10); canonical v4 승격

진단(`results/phase3/NEXT_STEPS_DIAGNOSIS.md`) 기반 4단계 실행. canonical:
`results/phase3/P_BLOCK_RESULTS.md`. 판정 4줄:

1. **P0-1 2024 확장**: test 2022-24, 사건 52→75 / 56→80 (+44%). 게이트
   전부 통과(v3 데이터의 ≤2023 부분집합 byte-동일 + anchor 재현). 2024는
   강한 해(0.75-0.80) → M-role 3년 종합 0.677/0.688 (모델 개선 아닌 추정
   갱신).
2. **P0-2 M_sa 채택** (M-role + 직전시즌 노출 + 경력 log + 구속결측 지표):
   H150 dROC **+0.017 [+0.001,+0.030] EXCL0** — 최초의 ROC-유의 개선,
   rolling 6/6 양. M_bf(백필)는 재검에서 기각 (프로브 기대 사망, 정직 기록).
3. **P1 미니 티어 전부 미채택**: prior_tjs(방향 양·보호 계수·evrec+4,
   CI 포함 → 사건 늘면 1순위 재검정), pitch-mix(유의 악화), torque(무효과),
   rest(dPR 유의 음수).
4. **P2 hazard supermodel 채택** = **canonical v4**: 30일 person-period +
   plain LR → **ROC 0.701 (H90) / 0.708 (H150)**, P_H90≤P_H150 정합 보장
   (이진 쌍은 47% 위반), 확률 보정 확보. role-stratified 미채택.

대표 수치 인용법: "3년(2022-24) 평균 ROC ~0.70, 연도 범위 0.62(2023)~
0.82(2024)". 데이터: `data/prospective/*_v3.parquet` (v2 보존).

**[갱신 2026-07-10 야간 — A1/B1 확장 + 사후 검증, 개선 캠페인 종료]**
test를 2022-25로 재확장(데이터 v4, 사건 94/100): **최종 대표 수치 =
hazard ROC 0.689 (H90) / 0.693 (H150) [0.64-0.74], recall@50 ~28%**.
2025는 보통 해(0.63-0.65) → 2024(0.80+)는 outlier로 확정 — 인용 시 "4년
평균 ~0.69, 통상 연도 0.63-0.68" 병기. 사후 검증(라벨 실재·as-of 무결·
2024 비의존성) 통과; **추정량 각주**: 문헌 비교용 스냅샷 ~0.69, "문헌
초과" 주장 금지. B1(경기 내 구속 감쇠) 기각, prior_tjs 3차 미채택(방향
양 지속 — KBO/시즌 축적 시 1순위), A2/B2 보류. 상세:
`results/phase3/P_BLOCK_RESULTS.md` A1+B1 절·검증 절.

**[정정 2026-07-13 — codex 외부 감사 수용, fable 재계산 검증]** 위 2022-25
수치는 **철회**: 2025 포함이 E0A의 라벨 신뢰 종료(2024-12-31) 결정 위반
(수집 미완결 연도의 누락 수술이 음성으로 오염). **정정 대표 수치 =
`t+H≤2024-12-31` 규칙, H90 ROC 0.701 [0.643,0.759] (사건 75) / H150
0.696 [0.645,0.746] (사건 80)** — "adaptive selection 후 조건부 backtest"
표기 필수. 추가 정정: hazard "이중계상 제거·확률 보정" 과장 철회(재보정
slope ~1.9, validation 재보정 필요), 무작위 대비 lift는 2-3×가 아니라
**1.55-1.76× (MC, 유의)**, "불펜 구조적 불가"는 "경보 배분 문제"로 완화
(RP-내 ROC 0.65, top-50 포착은 1건), KBO 기대치는 novel-pitcher ROC
**~0.66** 기준. 2025 창 = 동결 모델의 향후 전향 확인 세트로 재분류.
canonical: `results/phase3/P_BLOCK_RESULTS.md` 정정 헤더.
...
- **Phase 2.6**: 요약·결과 CSV·재현 스크립트 `results/phase26/`
  ...
  wf_554f0cb2(E3·검증).
- **Phase 3**: R 블록 `results/phase3/{R1_ROLLING,R2_AGE,R3_PREP,
  R4_KBO_LABELS}.md` + r1/r2 CSV; B 블록 `results/phase3/B_TIER_MODELS.md`
  + b_tier*/b_hgb_boundary CSV + `results/phase3/scripts/`.
